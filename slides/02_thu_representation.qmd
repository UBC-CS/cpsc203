...
## Color and Code {.smaller}

Make the poppies pop!

![](/images/poppyfield.jpg){fig-alt="A field of red poppies."}

Examine each pixel. If its red color channel is high (over 200, say), then
leave it as is. Otherwise, change the pixel to grey by setting all 3 color
channels to the average of their original values. 
...
''' Input: np.ndarray representing an image
    Output: an image
    Function: Create a new image by appling the boost_red_pixel function to every 
    pixel in the original, and saving the result to the copy.
'''
