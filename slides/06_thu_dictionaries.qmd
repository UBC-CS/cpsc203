...
* Examlet 2 is happening now through Thursday — good luck!
...
1. **The Intersection** — Membership pattern (`defaultdict(bool)`)
2. **The Cipher** — Counter pattern (`Counter`) + trial-and-error
3. **The Vault** — Complement/Two-Sum pattern
4. **The Alibi Map** — Grouping pattern (`defaultdict(list)`)

Today: The Translation/Mapping pattern + catch the thief!
...
On Tuesday, we cracked the cipher by trying shifts until we found English:

```
WKLV LV WKH FRGH: GLDJRQDO → THIS IS THE CODE: DIAGONAL
```

We found shift = 3 by trial and error. But what if we have a **much longer message**?

## Building a Decryption Dictionary

``` {pyodide}
# Build a complete letter → letter mapping
shift = 3
decrypt = {}
for i in range(26):
...
``` {pyodide}
# A much longer encrypted message from the thief's notebook
long_message = """
WKLV LV WKH VHFUHW SODQ: HQWHU WKURXJK WKH EDFN GRRU.
WDNH WKH JROGHQ GDWDIUDPH. HVFDSH EHIRUH PLGQLJKW.
"""

decoded = ""
for char in long_message:
    if char.isalpha():
        decoded += decrypt[char]  # Dictionary maps cipher → plain
    else:
        decoded += char
...
```python
# General form: build a mapping dictionary
mapping = {old_value: new_value for ...}

# Apply the mapping
result = [mapping[x] for x in data]
```

**Use when:** Converting codes, abbreviations, grades, categories...
...
2. **Room 2:** Cipher revealed: "DIAGONAL" — vault hint
3. **Room 3:** Vault code cracked
4. **Room 4:** Ms. Outlier has unverified alibi, no witnesses
5. **Room 5:** Escape route decoded — train station!
...
print(f"🔍 The thief is: {prime_suspect}")
```
...
- ✅ Was in both Gallery A and Gallery B
- ✅ Claims "faculty meeting" but unverified
- ✅ Another person claims same alibi (providing cover)
- ❌ Not verified to be elsewhere
...
1. **Record** — Store data with named fields (like Billboard songs)
2. **Membership** — Have I seen this before?
3. **Counting** — How many of each? (use `Counter`)
4. **Complement** — Find the matching pair
5. **Grouping** — Organize by category (use `defaultdict`)
6. **Translation** — Convert from one format to another
