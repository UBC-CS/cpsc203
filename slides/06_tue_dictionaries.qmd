...
* Examlet 2 is this Thursday — book your slot on PrairieTest!
...
try:
    ...
    print(f"KeyError: {e} — can't increment a key that doesn't exist!")
```
...
# Step 1: Count letter frequencies using Counter
...
## Room 2: Finding the Shift

``` {pyodide}
# Step 2: Most common letter in cipher (Counter makes this easy!)
most_common = letter_counts.most_common(1)[0][0]
print(f"Most common letter in cipher: {most_common}")

# Step 3: Assume it maps to 'E' - calculate shift
shift = ord(most_common) - ord('E')
print(f"Shift amount: {shift}")
```

## Room 2: Decoding with a Mapping Dictionary

``` {pyodide}
# Step 4: Build a decryption dictionary (mapping pattern!)
decrypt = {}
for i in range(26):
...
print("Decryption mapping:", decrypt)
```

## Room 2: Using the Mapping

``` {pyodide}
# Step 5: Decode using dictionary lookup
decoded = ""
for char in cipher:
    if char.isalpha():
...
This is the **translation/mapping pattern** — convert values using a dictionary!
...
try:
    ...
    print(f"KeyError: {e} — can't append to a list that doesn't exist!")
```
...
for suspect in suspects:
    if not suspect["verified"]:
        ...
        if not others:
            print(f"  {suspect['name']} claims '{alibi}' — NO WITNESSES!")
```
