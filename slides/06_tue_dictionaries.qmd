...
print(f"ord('W') = {ord('W')}")  # 87

# To get position in alphabet (0-25):
letter = 'W'
position = ord(letter) - ord('A')
print(f"'W' is at position {position} in the alphabet")  # 22
```

## Shifting Letters

``` {pyodide}
# To shift 'W' back by 3:
letter = 'W'
shift = 3

position = ord(letter) - ord('A')      # W is at position 22
new_position = (position - shift) % 26  # 22 - 3 = 19 (% 26 handles wrap-around)
new_letter = chr(new_position + ord('A'))  # Position 19 = 'T'

print(f"'{letter}' shifted back by {shift} = '{new_letter}'")
```
...
def decode_with_shift(text, shift):
    ...
    for char in text:
        if char.isalpha():
            # Convert to position, shift back, convert back to letter
