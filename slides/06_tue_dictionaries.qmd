...
``` {pyodide}
# Find alibis with only one person — no witnesses!
for alibi, names in by_alibi.items():
    if len(names) == 1:
        print(f"'{alibi}' has only one person: {names[0]} — NO WITNESSES!")
```
...
``` {pyodide}
# Cross-reference with unverified alibis
for suspect in suspects:
    if not suspect["verified"]:
        alibi = suspect["alibi"]
        if len(by_alibi[alibi]) == 1:
