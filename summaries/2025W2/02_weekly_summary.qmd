...
![Building squares from odd numbers: 1, then 1+3=4, then 1+3+5=9.](/images/odd-squares.png){fig-alt="Three squares showing 1×1, 2×2, and 3×3 grids with colored layers: red for 1, blue for 3, yellow for 5."}
...
![The k-th odd number is 2k−1, so we can work backwards to find k.](/images/finding-k.png){fig-alt="A table showing k maps to 2k-1, and the formula k = (last+1)/2 with example: 1+3+...+2381 has k=1191, so sum = 1191² = 1,418,481."}
...
![The 5×5 square sits in a corner of the 12×12, showing exactly which squares are subtracted.](/images/inclusion-exclusion.png){fig-alt="A 12×12 grid with a 5×5 corner highlighted in grey with a dashed red border, showing 12² − 5² = 119."}
...
![The sum of evens is the sum of odds plus k extra ones.](/images/evens-from-odds.png){fig-alt="A 3×3 grid for odds with a row of 3 colored +1 squares below, showing evens = odds + k."}
...
Plus, it's just satisfying when math has pictures! 🎨
