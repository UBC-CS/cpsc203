...
This week: [**Queues**]{.blue} + [**Flood Fill**]{.green} → **Efficient algorithmic art!**
...
![Tim Hortons Voronoi map of Vancouver](/images/tim-hortons-voronoi.png){fig-alt="A map of Vancouver with Tim Hortons locations and Voronoi region boundaries."}
...
::: {.column width="45%" style="padding-left: 1em;"}
...
![The Voronoi art pipeline: original photo → sample random centers → fill Voronoi regions](/images/voronoi-pipeline.svg){fig-alt="Three-step pipeline showing a photo being transformed: first we sample random points as centers, then each center claims nearby pixels to create Voronoi art." width="100%"}

Each center "claims" all nearby pixels, coloring them with that center's original color. More centers = more detail, but also more work!
...
To find the "nearest" center, we need to measure distance between points. The **Euclidean distance** between two points $(x_1, y_1)$ and $(x_2, y_2)$ comes from the Pythagorean theorem:

$$d = \sqrt{(x_2 - x_1)^2 + (y_2 - y_1)^2}$$

### Example: Which Center is Closer?

Suppose we have a pixel at $(5, 5)$ and two centers:
...
**Distance to Red:**
$$d_{\text{red}} = \sqrt{(5-2)^2 + (5-3)^2} = \sqrt{9 + 4} = \sqrt{13} \approx 3.6$$

**Distance to Blue:**
$$d_{\text{blue}} = \sqrt{(5-8)^2 + (5-6)^2} = \sqrt{9 + 1} = \sqrt{10} \approx 3.2$$

The pixel is closer to **Blue**!
...
::: {.callout-tip}
## A Clever Optimization: Skip the Square Root!

Computing square roots is slow. But here's the key insight: **if $a < b$, then $a^2 < b^2$** (for positive numbers).

So instead of comparing $\sqrt{13}$ vs $\sqrt{10}$, we can compare $13$ vs $10$ directly!
...
# Fast: squared distance (works for comparisons!)
def distance_squared(p1, p2):
    return (p2[0] - p1[0])**2 + (p2[1] - p1[1])**2
```

We only care about *which* center is closest, not the actual distance value. Since squaring preserves the ordering, we can skip the expensive `sqrt` call entirely!
:::
...
| Step | Work |
|------|------|
| 1. Read image | Proportional to $n$ — touch each pixel once |
| 2. Choose centers | Proportional to $k$ — pick k random points |
| 3. Build new image | Proportional to $n \cdot k$ — for each of n pixels, check all k centers |
| 4. Write image | Proportional to $n$ — touch each pixel once |

**Total: Proportional to $n \cdot k$** — this gets slow when we want lots of centers!
...
You could use `list.append()` and `list.pop(0)`, but `pop(0)` takes time proportional to $n$ because it shifts all remaining elements. With `deque`, both operations take constant time!
:::
...
| Step | Work |
|------|------|
| 1. Read image | Proportional to $n$ |
| 2. Choose centers | Proportional to $k$ |
| 3. Build new image | Proportional to $n$ — each pixel processed exactly once! |
| 4. Write image | Proportional to $n$ |

**Total: Proportional to $n$** — independent of the number of centers! 🎉
...
| Algorithm | Time | Good for |
|-----------|------|----------|
| Naive | Proportional to $n \cdot k$ | Few centers |
| Flood Fill | Proportional to $n$ | Many centers |
